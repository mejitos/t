...
case $COMMAND in
    help )
        ...
        exit 0;;
    test )
        build "$TEST_BUILD_DIR" "tests/main.c" "$TEST_BUILD_DIR/$TEST_EXECUTABLE" "tests/*.c src/lexer.c src/scope.c src/array.c src/type.c src/parser.c src/resolver.c src/interpreter.c src/stringbuilder.c src/ast.c src/token.c src/memory.c src/error.c"
