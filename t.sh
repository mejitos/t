...
case $COMMAND in
    help )
        ...
        exit 0;;
    test )
        build "$TEST_BUILD_DIR" "tests/main.c" "$TEST_BUILD_DIR/$TEST_EXECUTABLE" "tests/*.c src/lexer.c src/scope.c src/array.c src/parser.c src/resolver.c src/interpreter.c src/ir_generator.c src/ir_runner.c src/stringbuilder.c src/ast.c src/symbol.c src/type.c src/token.c src/memory.c src/error.c"
